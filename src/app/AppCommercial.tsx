...
export default function AppCommercial() {
  ...
  const handleAuthenticated = (user: User, subscription: Subscription, farm: Farm) => {
    setCurrentUser(user);
    setCurrentSubscription(subscription);
    setCurrentFarm(farm);
    setAppMode('app');
  ...
  const handlePaymentSuccess = () => {
    ...
    if (currentUser) {
      ...
      const updatedSubscription = subscriptions.find(s => s.userId === currentUser.id);
  ...
  // Payment Page
  if (appMode === 'payment' && currentUser && currentSubscription) {
    return (
      <PaymentPage
  ...
  // Main App - check subscription status
  if (!currentSubscription || !isSubscriptionActive(currentSubscription)) {
    // Subscription expired - force payment
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'people':
        return <People />;
      case 'settings':
        return <Settings currentUser={undefined} onUpdatePassword={() => {}} />;
      default:
        return <Dashboard />;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Subscription Banner */}
      {currentSubscription && (
        <SubscriptionBanner subscription={currentSubscription} onUpgrade={handleUpgrade} />
      )}
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600">{currentFarm?.name || 'Farm Management'}</p>
              </div>
            </div>
