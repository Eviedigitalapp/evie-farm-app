...
export function PaymentPage({
  ...
  const selectedNumber = paymentMethod === 'mtn-mobile-money' ? MTN_NUMBER : AIRTEL_NUMBER;
  const selectedNetwork = paymentMethod === 'mtn-mobile-money' ? 'MTN Mobile Money' : 'Airtel Money';
  ...
  const handleSubmitPayment = async () => {
    ...
    if (!subscription?.userId) {
      setErrorMessage('Your user account could not be identified. Please log out and log in again.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result?.error || `Payment API error ${response.status}: ${responseText.slice(0, 200)}`
        );
      }
      ...
      setErrorMessage(error?.message || 'Payment could not be submitted.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50 p-4">
        <div className="max-w-xl mx-auto py-12">
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-3">Payment Submitted</h1>
            <p className="text-gray-600 text-lg mb-6">Your payment information has been submitted to EVIE for verification.</p>
            ...
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 py-4 bg-green-600 text-white rounded-xl font-bold text-lg hover:bg-green-700 mb-3">💬 Notify EVIE on WhatsApp</a>
            <button onClick={() => { onSuccess?.(); onBack(); }} className="w-full py-3 bg-gray-100 text-gray-700 rounded-xl font-semibold hover:bg-gray-200">Back</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4">
      <div className="max-w-2xl mx-auto py-8">
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <button onClick={onBack} className="flex items-center gap-2 text-gray-600 hover:text-gray-800 mb-4"><ArrowLeft className="w-5 h-5" />Back</button>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Activate EVIE Agribusiness</h1>
          <p className="text-gray-600">Continue using EVIE after your 7-day free trial.</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <div className="space-y-3 mb-6">
            <button onClick={() => setPaymentMethod('mtn-mobile-money')} className={`w-full p-4 border-2 rounded-xl flex items-center justify-between ${paymentMethod === 'mtn-mobile-money' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200'}`}>
              <div className="flex items-center gap-3"><Smartphone className="w-7 h-7 text-yellow-600" /><div className="text-left"><p className="font-bold text-gray-900">MTN Mobile Money</p><p className="text-gray-600">{MTN_NUMBER}</p></div></div>
              {paymentMethod === 'mtn-mobile-money' && <CheckCircle className="w-6 h-6 text-green-600" />}
            </button>

            <button onClick={() => setPaymentMethod('airtel-money')} className={`w-full p-4 border-2 rounded-xl flex items-center justify-between ${paymentMethod === 'airtel-money' ? 'border-red-500 bg-red-50' : 'border-gray-200'}`}>
              <div className="flex items-center gap-3"><Smartphone className="w-7 h-7 text-red-600" /><div className="text-left"><p className="font-bold text-gray-900">Airtel Money</p><p className="text-gray-600">{AIRTEL_NUMBER}</p></div></div>
              {paymentMethod === 'airtel-money' && <CheckCircle className="w-6 h-6 text-green-600" />}
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div><label className="block font-semibold text-gray-700 mb-2">Phone Number Used for Payment *</label><input type="tel" value={paymentPhone} onChange={(e) => setPaymentPhone(e.target.value)} placeholder="+256 700 123 456" className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" /></div>
            <div><label className="block font-semibold text-gray-700 mb-2">Mobile Money Transaction Reference *</label><input type="text" value={transactionRef} onChange={(e) => setTransactionRef(e.target.value)} placeholder="Enter transaction reference" className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" /><p className="text-sm text-gray-500 mt-2">You can find the transaction reference in the Mobile Money confirmation SMS.</p></div>
          </div>
